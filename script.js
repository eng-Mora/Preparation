...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    // ── Build chapter HTML ────────────────────────────────────
    function buildChapter(chNum, chName, videos, code, studentCode) {
        const sorted = Object.entries(videos)
            .sort((a,b) => (a[1].order||0)-(b[1].order||0))
            .filter(([vid, v]) => {
                // لو مفيش accessCodes → يظهر للكل
                if (!v.accessCodes) return true;
                const codes = Object.values(v.accessCodes).map(String);
                // لو الـ accessCodes فاضية → يظهر للكل
                if (!codes.length) return true;
                // لو في أكواد → يظهر بس لو كود الطالب موجود
