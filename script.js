// ── Firebase compat SDK (loaded via CDN in index.html — iOS Safari safe) ──
...
// ── Wrapper helpers to keep same API style ────────────────
const ref     = (database, path) => database.ref(path);
const get     = (r)              => r.once('value');
const push    = (r, val)         => r.push(val);
const set     = (r, val)         => r.set(val);
const onValue = (r, cb)          => { r.on('value', cb); return r; };
const off     = (r)              => { try { if (r && r.off) r.off('value'); } catch(e) {} };

// ── Safe localStorage — لو iOS Safari رفضه مش هيوقف الصفحة ──
const _memStore = {};
const safeStorage = {
...
// ── تحويل Google Drive روابط للـ preview ─────────────────
...
// ── iframe HTML helper (iOS Safari compatible) ────────────
function makeIframe(url) {
    return `<div class="video-wrapper"><iframe src="${url}" frameborder="0" allow="autoplay *; fullscreen *; picture-in-picture *" allowfullscreen webkitallowfullscreen mozallowfullscreen referrerpolicy="no-referrer-when-downgrade"></iframe></div>`;
}
...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    function startBanWatcher(username) {
        if (banWatcherRef) off(banWatcherRef);
    ...
    async function checkLoginState() {
        try {
            ...
            const loginType = safeStorage.getItem('loginType');
