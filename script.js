// Safe localStorage wrapper for iOS Safari (private browsing / WebView)
var safeStorage = (function () {
    var _store = {};
    try {
        window.safeStorage.setItem('__test__', '1');
        window.safeStorage.removeItem('__test__');
        return {
...
// Firebase loaded via CDN scripts in index.html — use compat layer
// We load firebase-app-compat + firebase-database-compat via CDN
...
// Wrappers to match original API style
function dbRef(path)           { var db = _initFirebase(); return db ? db.ref(path) : null; }
function dbGet(path)           { var r = dbRef(path); return r ? r.once('value') : Promise.resolve({exists:()=>false,val:()=>null}); }
function dbSet(path, val)      { var r = dbRef(path); return r ? r.set(val) : Promise.resolve(); }
function dbPush(path, val)     { var r = dbRef(path); return r ? r.push(val) : Promise.resolve(); }
function dbOnValue(r, cb)      { if(r) r.on('value', cb); }
...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    function startBanWatcher(username) {
        if (banWatcherRef) dbOff(banWatcherRef); // امسح القديم لو موجود
        banWatcherRef = dbRef('students/' + username + '/banned');
        dbOnValue(banWatcherRef, (snap) => {
            if (snap.val() === true) {
                dbOff(banWatcherRef);
                safeStorage.removeItem('isLoggedIn');
                safeStorage.removeItem('username');
    ...
    async function getStudent(username) {
        try {
            const snap = await dbGet('students/' + username);
    ...
    async function getReviewByCode(code) {
        try {
            const snap = await dbGet('reviews');
    ...
    async function checkLoginState() {
        ...
        if (urlUser) {
            ...
            if (studentFromUrl && studentFromUrl.videoCode && !studentFromUrl.banned) {
                const now = Date.now();
                dbSet(dbRef(`students/${urlUser}/lastSeen`), now);
                dbPush(dbRef(`loginLogs/${urlUser}`), { at: now, name: studentFromUrl.name });
        ...
        if (loggedIn === 'true' && username) {
            ...
            if (student && student.videoCode) {
                ...
                dbSet(dbRef(`students/${username}/lastSeen`), Date.now());
    ...
    document.getElementById('loginForm').addEventListener('submit', async function (e) {
        ...
        if (student && student.videoCode) {
            ...
            // ── سجّل الدخول ──────────────────────────────────────
            const now = Date.now();
            dbSet(dbRef(`students/${username}/lastSeen`), now);
            dbPush(dbRef(`loginLogs/${username}`), { at: now, name: student.name });
            // ─────────────────────────────────────────────────────
            await loadVideoContent(student.videoCode, username, student);
            showMain();
            startBanWatcher(username);
    ...
    async function loadVideoContent(code, username, student) {
        ...
        let chapData = {};
        try {
            const snap = await dbGet('videos/' + code);
